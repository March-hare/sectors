 ...
 */
 ?>
    ...
    <?php print $geometries_hash; ?>
      ...
      if (geometry_id == 'add') {
        // Set the approved checkbox to one as all new geometries from
        // admin are automatically approved.
        $('input#approved').attr('checked', true);
      ...
      $('#geometryLabelerHolder').show().css('position', 'relative');
      ...
      // Event on Region select Change
      $('#geometries #sectors').change(function() {
        // clear the previous gemoetry
        // clear all the geometry forms
        clearMap(vlayer.features);

        // place the new geometry
        //console.log($('#geometries #sectors option:selected').val());
        geometry_id = $('#geometries #sectors option:selected').val();
        if (geometry_id != 'add') {
          createWKTFeature(geometry_id);
          updateForm(geometry_id);
          $('input#submit').attr('value', '<?php echo Kohana::lang('ui_main.modify'); ?>');
          ...
          $('input#approved').attr('checked', true);
          $('input#approved').attr('checked', true);
          $('input#submit').attr('value', '<?php echo Kohana::lang('ui_main.reports_btn_submit'); ?>');
    ...
    function updateForm(geometry_id) {
      geometry = geometries[geometry_id];
      $('input#geometry_label').attr('value', geometry['label']);
      $('input#geometry_comment').attr('value', geometry['comment']);
      $('input#geometry_color').attr('value', geometry['color']);
      $('input#geometry_strokewidth').attr('value', geometry['geometry_strokewidth']);
      $('input#approved').attr('checked', geometry['approved']);
      console.log(geometry['approved']);
    }
